...
func PauseBridge(v1BridgeConfig *util.V1BridgeConfig, config *util.Config) error {
	...
	if config.AutoPauseBridge {
		...
		for _, chain := range v1BridgeConfig.Chains {
			...
			err := pauseBridgeContract(chain, pk)
			if err != nil {
				fmt.Println(err)
			}
		}
...
func pauseBridgeContract(chain util.RawChainConfig, pk string) error {
	fmt.Printf("Paused bridge contract on chain %s", chain.Name)
	return nil
}
